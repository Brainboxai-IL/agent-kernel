...
Every rule in `modules/` exists because its absence produces a specific, observed failure mode in LLM agents. This document records the failure each rule prevents — so rules can be evaluated, challenged, and pruned instead of accumulating as folklore.
...
| C7 Marked uncertainty | The classic laundering chain: "probably X" in reasoning becomes "X" in the summary and a production decision by Friday. |
...
| A2 Proceed/ask boundary | "Shall I proceed?" on a reversible step of the assigned task — which, in unattended runs, silently kills the session. |
...
| A5 Evidence before state changes | The signature-match restart: symptom resembles last week's incident, so the agent restarts the service — for a different root cause, destroying the evidence. |
...
| I2 No fabrication | Invented API parameters, phantom config keys, benchmark numbers that were never measured — stated at full confidence. |
...
| Rule | Failure mode it prevents |
|------|--------------------------|
| S1 Irreversibility gate | The one-way door crossed casually: force-push, prod delete, mass email — performed at the same confidence as a file read. |
...
| S3 External means published | Secrets or drafts sent to an external service "temporarily" — and cached, logged, or indexed permanently. |
...
| S6 Blast-radius containment | Running the batch on all 4,000 rows first try, when one row would have revealed the mapping bug. |
...
| K2 Comments state constraints | `// increment counter`, `// this change fixes the bug` — reviewer-directed noise that rots the moment it merges. |
| K3 Smallest correct change | The 40-line fix arriving inside a 400-line opportunistic refactor no one asked for or can review. |
